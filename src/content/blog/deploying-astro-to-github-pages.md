...
This lives at `.github/workflows/deploy.yml`. It builds the site and publishes
it using the official Astro action:
...
Here's the part that trips people up. A custom domain is stored in a `CNAME`
file, and a fresh build can wipe it out. The fix is to keep `CNAME` in Astro's
`public/` directory, which gets copied straight into `dist/` on every build:
...
Because it ships with every build, the domain never drops, and you don't have
to re-enter it after each deploy.
...
*(This is an example post showing off code highlighting. Delete it whenever.)*
